    ...
    const handleRetry = useCallback(
        async (batchId: string) => {
            ...
            try {
                ...
                if (result.success) {
                    notifySuccess('Retry successful', `Batch ${batchId} has been re-submitted to the network.`);
                            ...
                            b.id === batchId
                                ? {
                                    ...
                                    recipients: b.recipients.map((r: BatchRecipient) =>
                                        r.status === 'failed' ? { ...r, status: 'pending' as const, errorMessage: undefined } : r
                                    ),
                                }
                                : b
                        )
                    ...
                    notifyError('Retry failed', result.error ?? 'The retry attempt was rejected by the network.');
                }
            } catch (err) {
