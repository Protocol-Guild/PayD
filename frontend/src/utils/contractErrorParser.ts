...
const CONTRACT_ERROR_MAPPING: Record<number, { message: string; action: string }> = {
    ...
    5: {
        ...
        action: 'The batch exceeds the maximum allowed size (100). Please split it into smaller batches.',
    },
    6: {
    ...
    7: {
        ...
        action: 'The total batch amount exceeds the capacity of the contract. Please reduce the amounts.',
    },
    8: {
 ...
 */
export function parseContractError(resultXdr?: string, simulationError?: string): ContractErrorDetail {
                        ...
                        if (tr.switch() === xdr.OperationType.invokeHostFunction()) {
                            ...
                            if (ihfCode !== xdr.InvokeHostFunctionResultCode.invokeHostFunctionSuccess()) {
                                return {
                                    ...
                                    suggestedAction: 'Review the contract state, resource limits, and input parameters.',
                                    rawXdr: resultXdr,
                                };
                            }
                        }
    ...
    return {
        ...
        suggestedAction: 'Check your network connection and try again, or contact support if the issue persists.',
        rawXdr: resultXdr,
    };
}
