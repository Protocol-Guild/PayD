  ...
  static async getContracts(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (typeof network === 'string' && network.length > 0) {
        const contracts = ContractRegistryService.getContractsByNetwork(network);
      ...
      if (responseTime > 500) {
        logger.warn(`Contract registry response slow: ${responseTime}ms`);
      }
      ...
      res.status(500).json({
        ...
        message: error instanceof Error ? error.message : 'Failed to load contract registry',
        timestamp: new Date().toISOString(),
      });
    }
  }
