...
describe('Contract Registry API Integration', () => {
  ...
  describe('GET /api/contracts', () => {
    it('returns all networks when no query param provided', async () => {
      (ContractRegistryService.getAllContracts as jest.Mock).mockReturnValue({
        testnet: {
          bulk_payment: {
    ...
    it('returns specific network when query param is provided', async () => {
      (ContractRegistryService.getContractsByNetwork as jest.Mock).mockReturnValue({
        bulk_payment: {
          contractId: 'CBULK_TEST',
          version: '1.0.0',
          deployedAt: 123456,
      ...
      const response = await request(app).get('/api/contracts').query({ network: 'testnet' });
    ...
    it('returns 500 when service throws error', async () => {
      (ContractRegistryService.getAllContracts as jest.Mock).mockImplementation(() => {
