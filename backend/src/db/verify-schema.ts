...
if (!DATABASE_URL) {
  ...
  console.error(
    '[verify-schema] Please create a .env file in the backend directory with DATABASE_URL.'
  );
  process.exit(1);
}
...
async function verifySchema(): Promise<void> {
  ...
  try {
    ...
    const executionHistoryConstraintTypes = executionHistoryConstraints.rows.map(
      (r) => r.constraint_type
    );
    ...
    console.error(
      '\n[verify-schema] ✗ Schema verification failed:',
