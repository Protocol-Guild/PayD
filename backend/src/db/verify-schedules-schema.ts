  ...
  try {
    ...
    for (const expected of expectedSchedulesColumns) {
      ...
      if (!actual) {
        ...
        console.error(
          `✗ Column '${expected.name}' has wrong type: ${actual.data_type} (expected ${expected.type})`
        );
        allChecksPass = false;
      } else if (actual.is_nullable !== expected.nullable) {
        console.error(
          `✗ Column '${expected.name}' has wrong nullable: ${actual.is_nullable} (expected ${expected.nullable})`
        );
        allChecksPass = false;
      } else {
        console.log(
          `✓ Column '${expected.name}' is correct (${expected.type}, nullable: ${expected.nullable})`
        );
      }
    }
    ...
    const expectedSchedulesChecks = [
      {
        name: 'schedules_frequency_check',
        pattern: /frequency.*IN.*once.*weekly.*biweekly.*monthly/i,
      },
      {
        name: 'schedules_status_check',
        pattern: /status.*IN.*active.*completed.*cancelled.*failed/i,
      },
    ];
    ...
    for (const expected of expectedSchedulesChecks) {
      ...
      if (!actual) {
        ...
        console.error(
          `✗ CHECK constraint '${expected.name}' has wrong definition: ${actual.check_clause}`
        );
        allChecksPass = false;
      } else {
    ...
    for (const expected of expectedExecutionHistoryChecks) {
      const actual = executionHistoryConstraints.rows.find(
        (c) => c.constraint_name === expected.name
      );
      if (!actual) {
        console.error(`✗ CHECK constraint '${expected.name}' is missing`);
        allChecksPass = false;
    ...
    for (const expected of expectedForeignKeys) {
      ...
      if (!actual) {
        console.error(
          `✗ Foreign key ${expected.table}.${expected.column} -> ${expected.foreign_table}.${expected.foreign_column} is missing`
        );
        allChecksPass = false;
      } else {
        console.log(
          `✓ Foreign key ${expected.table}.${expected.column} -> ${expected.foreign_table}.${expected.foreign_column} exists`
        );
      }
    }
    ...
    if (!orgsExists.rows[0].exists) {
      ...
      try {
        ...
        await client.query('ROLLBACK');
        console.error(
          '✗ Foreign key constraint execution_history.schedule_id -> schedules.id is NOT enforced'
        );
        allChecksPass = false;
      } catch (err) {
        await client.query('ROLLBACK');
